...
namespace Bolsover.Export
{
    public class StepWriter
    {
        ...
        private EdgeCurve CreateEdgeCurve(Vertex vertex1, Vertex vertex2, bool isForward, double tolerance)
        {
            ...
            return Register(new EdgeCurve(NextId(), vertex1, vertex2, line1, isForward));
            ...
            ShellModel.ShellModelTypes modelType)
        {
            ...
            if (modelType == ShellModel.ShellModelTypes.Solid)
            {
                // SOLID

                // units
                var lengthUnit = Register(new Unit(NextId(), Unit.UnitTypes.Length));
                var planeAngleUnit = Register(new Unit(NextId(), Unit.UnitTypes.PlaneAngle));
                var solidAngleUnit = Register(new Unit(NextId(), Unit.UnitTypes.SolidAngle));
